...
function buildLogoWithBackground(logo: Buffer): Promise<Buffer> {
  return sharp({
    create: {
      width: 220,
      height: 220,
...
function buildAdversaireLines(
  ...
  const secondLine = line2
    ? `<text x="${WIDTH / 2}" y="${baseY + fontSize + 4}" font-size="${fontSize}" fill="#1a1a1a" font-weight="bold" text-anchor="middle"
        font-family="'Bebas Neue', 'Arial Black', sans-serif" letter-spacing="3">${line2}</text>`
    : "";

  return `
    <text x="${WIDTH / 2}" y="${baseY}" font-size="${fontSize}" fill="#1a1a1a" font-weight="bold" text-anchor="middle"
      font-family="'Bebas Neue', 'Arial Black', sans-serif" letter-spacing="3">${line1}</text>
    ${secondLine}
  `;
}

function buildSingleAdversaireSvg(
  adv1Line1: string,
  adv1Line2: string | null,
  fontSize1: number,
  isHome: boolean,
  salle: string
): string {
  const baseY = adv1Line2 ? 845 : 860;
  const iconY = adv1Line2 ? 845 + fontSize1 * 2 + 10 : 900;
  const salleY = adv1Line2 ? 845 + fontSize1 * 2 + 55 : 950;
  const homePath = "M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z";
  const awayPath =
    "M21 16v-2l-8-5V3.5c0-.83-.67-1.5-1.5-1.5S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z";

  return `
    ${buildAdversaireLines(adv1Line1, adv1Line2, fontSize1, baseY)}
    <g transform="translate(${WIDTH / 2 - 16}, ${iconY})">
      <svg width="32" height="32" viewBox="0 0 24 24">
        <path d="${isHome ? homePath : awayPath}" fill="#CC1E1E"/>
      </svg>
    </g>
    <text x="${WIDTH / 2}" y="${salleY}" font-size="20" fill="#666666" font-weight="normal" text-anchor="middle"
      font-family="'Montserrat', Arial, sans-serif">${salle}</text>
  `;
}

function buildDoubleAdversaireSvg(
  adv1Line1: string,
  adv1Line2: string | null,
  fontSize1: number,
  adv2Line1: string,
  adv2Line2: string | null,
  fontSize2: number
): string {
  const adv1BaseY = adv1Line2 ? 848 : 858;
  const afterAdv1Y = adv1Line2 ? 848 + fontSize1 * 2 + 8 : 858 + fontSize1;
  const ampersandY = afterAdv1Y + 8;
  const adv2BaseY = ampersandY + 30;
  ...
  return `
    ...
    ${buildAdversaireLines(adv1Line1, adv1Line2, fontSize1, adv1BaseY)}
    <text x="${WIDTH / 2}" y="${ampersandY}" font-size="20" fill="#CC1E1E" font-weight="bold" text-anchor="middle"
      font-family="'Montserrat', Arial, sans-serif">&amp;</text>
    ${buildAdversaireLines(adv2Line1, adv2Line2, fontSize2, adv2BaseY)}
  `;
}
...
export async function generateUpcomingVisual(
  ...
  const logo = await sharp(path.join(ASSETS_DIR, "logo.png"))
    .resize(200, 200)
    .toBuffer();
  const logoWithBackground = await buildLogoWithBackground(logo);
  ...
  const adversairesSvg = hasMultiple
    ...
    : buildSingleAdversaireSvg(adv1Line1, adv1Line2, fontSize1, isHome, salle);
